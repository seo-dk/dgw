...
from commons_test.DistcpExecutor import DistcpExecutor, DistcpStatus, DistcpConfig
from commons_test.ProvideMetaInfoHook import ProvideMetaInfoHook, ProvideInterface
...
class ProvideDistcpManager:
    def __init__(self, context, collect_interface_info=None, provide_file_info: CollectProvideInfo=None, src_path=None, retry=None,
        ...
        # if provide_file_info exist it means that it is called after finished collect
        # if provide_interface exist it means that it is called after od triggered

        self.provide_meta_info_hook = ProvideMetaInfoHook()
        self.collect_interface_info = collect_interface_info

        # at here it is triggered
        if context:
            self._init_context_vars(context)
        else:
            # but in here it is not triggered
    ...
    def _init_context_vars(self, context):
        # need to change this part because we will make dag by dag template code
        # we need to check if it is triggered or not
        dag_run = context['dag_run']
        trigger = dag_run.conf.get('trigger')
        if dag_run.conf.get('trigger'):
            # at here we need to check the provide infos
    ...
    def _calculate_dt_pattern(self, target_dir, partitions_dict):
        ...
        # test
        processed_target_dir = "hdfs://" + "/".join(processed_dir_parts) + "/test"
