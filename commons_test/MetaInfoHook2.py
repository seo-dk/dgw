...
class DistcpSourceProtoInfo:
    ...
    header_skip: bool = False
    enc_rules: List[Dict[str, List[int]]] = None
...
class InterfaceInfo:
    ...
    system_cd: str = None
    proto_info: str = None
    ftp_proto_info: FtpInterfaceProtoInfo = None
    dbonn_proto_info: DbconnInterfaceProtoInfo = None
...
class CollectProvideInfo:
    ...
    target_id: str = None
    provide_hdfs_dir: str = None

def map_json_to_class(cls, json_data):
    ...
    filtered_data = {k: v for k, v in json_data.items() if k in cls_fields}
    return cls(**filtered_data)

protocol_class_map = {
    'FTP': FtpInterfaceProtoInfo,
    'SFTP': FtpInterfaceProtoInfo,
    'DBCONN': DbconnInterfaceProtoInfo,
    'DISTCP': DistcpInterfaceProtoInfo,
    'KAFKA': KafkaInterfaceProtoInfo,
}
...
class MetaInfoHook:
    ...
    def _get_interface_proto_info(self, cls, json_data):
        return map_json_to_class(cls, json_data)

    def _get_ftp_interface_proto_info(self, json_data):
        return map_json_to_class(FtpInterfaceProtoInfo, json_data)
        # return FtpInterfaceProtoInfo(ip=json_data['ip'], port=json_data['port'],
        #                              local_dir=json_data['local_dir'], id=json_data['id'],
        #                              pwd=json_data['pwd'], source_dir=json_data['source_dir'],
    ...
    def _get_ftp_source_proto_info(self, json_data, source_file_nm):
        proto_info = map_json_to_class(FtpSourceProtoInfo, json_data)
        proto_info.source_file_nm = source_file_nm
        return proto_info
        ...
        #                           use_regex_filename=json_data.get('use_regex_filename'), encrypt_type=json_data.get('encrypt_type'),
        #                           encrypt_column=json_data.get('encrypt_column'), header_skip=json_data.get('header_skip', False),
        ...
        #                           prevent_split=json_data.get("prevent_split"))

    def _get_dbconn_interfce_proto_info(self, json_data):
        return map_json_to_class(DbconnInterfaceProtoInfo, json_data)    
    ...
    def _get_dbconn_source_proto_info(self, json_data, source_file_nm):
        proto_info = map_json_to_class(DbconnSourceProtoInfo, json_data)
        proto_info.source_file_nm = source_file_nm
        return proto_info
    ...
    def _get_distcp_interface_proto_info(self, json_data):
        return map_json_to_class(DistcpInterfaceProtoInfo, json_data)
        ...
        #                                 delimiter=json_data.get('delimiter', '\036'))

    def _get_distcp_source_proto_info(self, json_data, source_file_nm):
        proto_info =  map_json_to_class(DistcpSourceProtoInfo, json_data)
        proto_info.source_file_nm = source_file_nm
        return proto_info
        ...
        #                              enc_rules=json_data.get('enc_rules'), has_header=json_data.get('has_header', False), source_file_pattern=json_data.get('source_file_pattern'),
        #                              enc_memory_usage=json_data.get('enc_memory_usage', '25g'))
    ...
    def get_interface_info(self, interface_id) -> InterfaceInfo:
        try:
            ...
            if not record:
                logger.warning(f"no record, not exist {interface_id}")
                return None

            self.interface_info = InterfaceInfo(*record)            
            json_data = json.loads(self.interface_info.proto_info)

            proto_cls = protocol_class_map.get(self.interface_info.protocol_cd)                
            if proto_cls:
                proto_info_attr = f"{self.interface_info.protocol_cd.lower()}_proto_info"            
                setattr(self.interface_info, proto_info_attr, self._get_interface_proto_info(proto_cls, json.loads(self.interface_info.proto_info)))
            else:
                logger.warning("Unknown protocol_cd: %s", self.interface_info.protocol_cd)

            # if self.interface_info.protocol_cd == 'FTP' or self.interface_info.protocol_cd == 'SFTP':                
            #     self.interface_info.ftp_proto_info = self._get_interface_proto_info(FtpInterfaceProtoInfo, json_data)
            ...
            #     self.interface_info.db_proto_info = self._get_interface_proto_info(DbconnInterfaceProtoInfo, json_data)
            ...
            #     self.interface_info.distcp_proto_info = self._get_interface_proto_info(DistcpInterfaceProtoInfo, json_data)
            ...
            #     self.interface_info.kafka_proto_info = self._get_interface_proto_info(KafkaInterfaceProtoInfo, json_data)
    ...
    def get_dbconn_source_file_info(self, inf, seq_num):
        try:
            ...
            #     self._check_mapping_with_provide_table(file_info)
    ...
    def _check_mapping_with_provide_table(self, source_info):
        if self.get_collect_provide_infos(source_info.interface_id, source_info.collect_source_seq):
            source_info.isMappedWithProvideTable = True

    def get_collect_provide_infos(self, interface_id, seq_num=None):
        try:
            # test 위해 use_yn = Y 제거
            sql = """SELECT ci.proto_info->>'$.delimiter' as c_del, cs.interface_id, cs.hdfs_dir, 
                p.interface_id, p.proto_info->>'$.delimiter' as p_del, p.proto_info->>'$.target_dir' as target_dir, 
                p.protocol_cd, p.target_id, CONCAT('/db=', p.db, '/tb=', p.tb) as provide_hdfs_dir
                FROM collect_source cs
                JOIN collect_interface ci
                on cs.interface_id = ci.interface_id
                JOIN provide_interface p
                on cs.hdfs_dir = CONCAT('/db=', p.db, '/tb=', p.tb)
                JOIN provide_target pt
                on pt.target_id  = p.target_id
                WHERE p.run_by = 'collect'                 
                and p.interface_id not like '%%-TEST-%%'
                and cs.interface_id = %s"""
...
if __name__ == "__main__":
    ...
    interface_info = meta_info_hook.get_interface_info('C-IAM-FTP-DD-0001')
    logger.info("Result: %s", interface_info)
