...
from commons_test.MetaInfoHook import SourceInfo, InterfaceInfo, SparkServerInfo
import commons_test.HdfsUtil as HdfsUtil
...
class DistcpParquetHandler:
    def __init__(self, source_info: SourceInfo, interface_info: InterfaceInfo, hdfs_src_path):
        ...
        ## self.hdfs_src_path : dataGW 에서 distcp 수집한 경로.
    ...
    def _convert_in_local(self):
        # source_hdfs_path down to local path
        # hdfs://dataGWCluster/staging/idcube_out/db=o_tpani_cem5g/tb=PGW_1H/dt=
    ...
    def _download(self, src_path, dest_path):
        ...
        # hdfs://dataGWCluster/staging/idcube_out/db=o_tpani_cem5g/tb=PGW_1H
        # /data/idcube_out/db=o_tpani_cem5g/tb=PGW_1H
        ...
        # hdfs://dataGWCluster/staging/decomp/idcube_out/db=o_tpani_cem5g/tb=PGW_1H
        hdfs_decomp_path = src_path.replace("/staging", "/staging/decomp")

        if self.source_info.distcp_proto_info.format == 'lz4':
            # if not self._check_hdfs_path(hdfs_decomp_path):
            #     self._mkdir_to_hdfs(hdfs_decomp_path)
            ...
            # upper_dir_dest_path = dest_path.rsplit('/', 1)[0]
