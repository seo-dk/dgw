...
class HdfsCopy:
    def __init__(self, kafka_send, collect_history: CollectHistory, delimiter, port=8020, user='hadoop'):
        ...
        try:
            # self._hdfs = HadoopFileSystem(self.host, port=self.port, user=self.user)
    ...
    def _copy_local_to_hdfs(self, local_path, dest, encoding):
        ...
        with open(local_path, 'r', encoding=encoding, errors='replace') as local_file:
            hdfs = HadoopFileSystem(self.host, port=self.port, user=self.user)
    ...
    def copy(self, args):
        local_path, dest, ulid, stdout, retry, db_encoding_info, collectHistory = args
        ...
        try:
            ...
            if self.compression_manager.check_gz_extension(local_path) or local_path.endswith(".zip"):
                ...
                if local_path.endswith(".parquet"):
                    ...
                    self._copy_local_to_hdfs(local_path, dest, encoding)
    ...
    def main(self, collect_history, info_dat_path, encoding=None, stdout=False, retry=False):
        ...
        with Pool(processes=5) as pool:
            ...
            results = pool.map(self.copy, tasks, chunksize=1)
            pool.close()
            pool.join()
