...
from commons_test.FtpSftpManager import FtpSftpManager, CollectedFileInfo
...
class FileCollectManager:
    ...
    def trigger_test(self):
        files_path = "/data/idcube_in/db=test/tb=test/dt=20240709/"
        # need to get file list from files_path
    ...
    def _remove_file(self, collected_file_infos: List[CollectedFileInfo]):
        for collected_file_info in collected_file_infos:
            ...
            if collected_file_info.source_info.file_proto_info.crypto_rules.enabled and collected_file_info.source_info.file_proto_info.crypto_rules.store_raw_and_crypto:
                staging_local_path = str(local_path).replace("/data/test/idcube_out", "/test/staging/raw/idcube_out")
                staging_hadoop_path = str(hadoop_path).replace("/test/idcube_out", "/test/staging/raw/idcube_out")
