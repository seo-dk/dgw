...
from commons_test.MetaInfoHook import SparkServerInfo, ColumnCryptoInfo
from commons_test.VaultClient import get_vault_token, VAULT_IP, VAULT_ADDR, VAULT_NAMESPACE, VAULT_SECRET_PATH
...
class DistcpEncryptManager:
    ...
    def extract_tags_from_input(self, local_path):
        ...
        def ensure_output_path(output_path):
            if self.is_encrypted and self.store_raw_and_crypto:
                return output_path.replace("/test", "/test/staging/raw")
            return output_path
        ...
        if hh_match:
            hh = hh_match.group(1)
            output_path = f"/test/idcube_out/db={db}/tb={tb}/dt={dt}/hh={hh}"
            return ensure_output_path(output_path)
        if hm_match:
            hm = hm_match.group(1)
            output_path = f"/test/idcube_out/db={db}/tb={tb}/dt={dt}/hm={hm}"
            return ensure_output_path(output_path)
        output_path = f"/test/idcube_out/db={db}/tb={tb}/dt={dt}"
        return ensure_output_path(output_path)
    ...
    def _exec(self, vault_token, input_file_pattern, output_path, file_exist):
        ...
        if file_exist:
            logger.info(f"CryptoConfig : {self.crypto_config}")
            ...
            exec_command = (f'cd "{self.crypto_config.encrypt_server_info.exec_file_path}"\n'
                            ...
                            f'sh run_data_processing_test.sh "{input_file_pattern}" "{output_path}" "{self.tb}"')
