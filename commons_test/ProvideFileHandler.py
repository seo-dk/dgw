...
from commons_test.ConvertToCsv import ConvertToCsv
from commons_test.ProvideFormatConverter import ProvideFormatConverter
from commons_test.MetaInfoHook import SourceInfo, InterfaceInfo
import commons_test.HdfsUtil as hu
    ...
    def set_crypto_provide_path(self):
        ...
        if not self.provide_interface.proto_info_json.spark_server_info.is_external:
            if "/staging/raw" in self.src_hdfs_full_path and not self.provide_interface.proto_info_json.use_raw_file:
                ...
                self.src_hdfs_full_path = self.src_hdfs_full_path.replace("/test/idcube_out", "/test/staging/raw/idcube_out")
                self.src_hdfs_full_path = remove_file_name_in_path(self.src_hdfs_full_path)
