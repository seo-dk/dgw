...
from commons_test.HdfsUtil import put_files, make_directory
...
class FtpSftpCryptoExecutor:
    ...
    def start(self):
        if self.source_info.file_proto_info.enc_rules:
            crypto_executor = CryptoExecutor(self.is_header_skip, self.source_info.file_proto_info.enc_rules,self.local_path_infos.local_tmp_full_path,
                                             self.interface_info.ftp_proto_info.encode, self.interface_info.ftp_proto_info.delimiter, True)
            return crypto_executor.start_encrypt_and_decrypt()
    ...
    def _start_encrypt_decrypt(self, crypto_rules, is_encrypt):
        try:
            store_raw_and_encrypted = next((crypto_rule for crypto_rule in crypto_rules if "enc_dec_enable" in crypto_rule), None)
            logger.info(f"enc_dec_enable : {store_raw_and_encrypted}")
            if store_raw_and_encrypted and store_raw_and_encrypted.get("enc_dec_enable") is False:
    ...
    def _upload_raw_data_to_hadoop(self):
        ...
        staging_hdfs_path = self.partitions.get("hdfsPath").replace("/test/idcube_out", "/staging/raw/test/idcube_out")
        staging_local_path = str(self.local_path_infos.local_tmp_full_path).strip().replace("/data/test/idcube_out", "/data/staging/raw/test/idcube_out")
