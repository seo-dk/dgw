...
# from commons_test.HdfsCopy_v3 import check_dir, main
from commons_test.KafkaSend import KafkaSend
from commons_test.MetaInfoHook import MetaInfoHook, SourceInfo
from commons_test.PathUtil import PathUtil
from commons_test.ProvideManager import ProvideManager
from commons_test.GetDbConnInfoForCollectHistory import GetDbConnInfoForCollectHistory
...
class DbConnManager:
    ...
    def start(self):
        ...
        try:
            ...
            get_db_conn_collect_hist = GetDbConnInfoForCollectHistory(collect_history, interface_info)

            handle_db_conn_list = HandleDbConnList(self.formatted_execution_date, get_db_conn_collect_hist, kafka_send, collect_history, interface_info, source_infos, local_info_file_path,
                                                   self.collect_file_info_list)

            handle_db_conn_list.handle_file_list()

            if local_info_file_path is not None:
                # hdfs_file_uploader = HdfsFileUploader(kafka_send, collect_history, '')
                # hdfs_file_uploader.check_dir(local_info_file_path)
                # hdfs_file_uploader.start(local_info_file_path, stdout=True, retry=self.retry)
