...
TEST_TOPIC = "hdfs_load_completion_test"


class KafkaSend:
    def __init__(self):
        self.producer = self.set_kafka_config()
        self.partition_count = len(self.producer.partitions_for(TEST_TOPIC))
    ...
    def send_to_kafka(self, collect_history):
        try:
            ...
            jsondata = json.dumps(asdict(collect_history))
            topic = TEST_TOPIC
            self.producer.send(topic, jsondata, partition=partitionNum)
