...
from commons_test.ParquetInfoManager import ParquetInfoManager
...
def convert(csv_file_pattern, source_info: SourceInfo, interface_info: InterfaceInfo, collect_history: CollectHistory, info_dat_path):
    # csv_files = _decompress_gz_file(glob.glob(csv_file_pattern))
    csv_files = _decompress_gz_file(glob.glob(csv_file_pattern))
    if not csv_files:
        logger.warning("No CSV files found matching the pattern.")
        return

    ## get a total size of a files
    ...
    # convert_files(source_info, interface_info, collect_history, info_dat_path, csv_files, True)
    #
    if average_size_mb >= 9 and parallel_flag:
        logger.info("Using parallel processing due to large file sizes.")
        convert_files(source_info, interface_info, collect_history, info_dat_path, csv_files, True)
    else:
        logger.info("Using sequential processing due to smaller file sizes.")
        convert_files(source_info, interface_info, collect_history, info_dat_path, csv_files)


def convert_files(source_info: SourceInfo, interface_info: InterfaceInfo, collect_history: CollectHistory, info_dat_path, local_files, parallel=False):

    logger.info("local_files before convert : %s", local_files)

    ## until here it is getiing schema info from it
    try:
        ## it it need parallel go in to here.
        ...
        if parallel:
            ...
            with Pool(processes=worker_num) as pool:
                ...
                local_files_parquet_msgs_arr = pool.starmap(_convert, [(local_file, source_info, interface_info, schema_dict,
                                                                        parallel) for local_file in local_files], chunksize=1)
                # pool.close()
                # pool.join()
            if interface_info.protocol_cd in ['FTP', 'SFTP', 'DBCONN']:
                logger.info("writing result info_dat_file : %s", info_dat_path)
                if _get_merge(interface_info, source_info):
                    local_files_parquet_msgs_arr = _concat_parquet_files(local_files_parquet_msgs_arr)
                for local_files_parquet_msgs in local_files_parquet_msgs_arr:
                    if info_dat_path:
                        _send_message(local_files_parquet_msgs, source_info, interface_info, collect_history, info_dat_path)

                end_time = time.time()
        else:
            logger.info("working as sequential")
...
def _convert(local_files, source_info: SourceInfo, interface_info: InterfaceInfo, schema_dict, parellel = False, parquet_size_mb=2000):
    ...
    try:
        ...
        for local_file in local_files:
            writer, local_parquet_files = _convert_and_write(base_name, local_file, parquet_size_mb, writer, converters,
                                                             delimiter, compression, schema_dict, encode, merge, parellel)
...
def _convert_and_write(base_name, local_file, parquet_size_mb, writer, converters, delimiter, compression, schema_dict, encoding, merge, parellel):
    local_parquet_files = []
    ...
    csv_reader = pd.read_csv(local_file, header=None, names=field_names,
                             ...
                             na_values=['', ' '], engine='python', chunksize=chunksize)
    ...
    try:
        if not os.path.exists(parquet_dir):
            logger.info("parquet_dir does not exist")
            os.makedirs(parquet_dir)
    except FileExistsError as e:
        logger.warning("directory exist : %s",e)
...
def _get_merge(interface_info: InterfaceInfo, source_info: SourceInfo):
    if interface_info.protocol_cd in ['FTP', 'SFTP']:
...
def _send_message(local_files_parquet_msgs, source_info: SourceInfo, interface_info: InterfaceInfo, collect_history: CollectHistory, info_dat_path):
    for local_file, parquet_files in local_files_parquet_msgs.items():
        for parquet_local_file in parquet_files:
            logger.info("_send_message : %s", parquet_local_file)
...
def _decompress_gz_file(local_files):
    ...
    decompress_result_files = []
    for local_file in local_files:
        if ".gz" in local_file:
            decompress_file = compression_manager.decompress_gz_with_zlib(local_file)
            decompress_result_files.append(decompress_file)
    if decompress_result_files:
        return decompress_result_files
    else:
        return local_files


def _make_merge_parquet_path(parquet_full_path):
    logger.info("parquet_full_path : %s",parquet_full_path)
    tb_value = re.search(r'/tb=([^/]+)/', str(parquet_full_path)).group(1)
    new_path = re.sub(r'/([^/]+)\.([^/]+)\.parquet$', f'/{tb_value}.\\2.parquet', str(parquet_full_path))
    logger.info("new parquet_path : %s",new_path)
    return new_path


def _concat_parquet_files(local_files_parquet_msgs_arr):
    dataframes = []
    merged_parquet_file = None
    for local_files_parquet_msgs in local_files_parquet_msgs_arr:
        for local_file, parquet_files in local_files_parquet_msgs.items():
            for parquet_local_file in parquet_files:
                if merged_parquet_file is None:
                    merged_parquet_file = _make_merge_parquet_path(parquet_local_file)
                dataframes.append(pd.read_parquet(parquet_local_file))
    merged_df = pd.concat(dataframes, ignore_index=True)
    ## need to make parquet file path at here
    logger.info("merged_parquet_file : %s", merged_parquet_file)
    merged_df.to_parquet(merged_parquet_file, compression='zstd')
    merged_parquet_file_msg = [{'merged_parquet_file': [merged_parquet_file]}]
    return merged_parquet_file_msg
