...
class MetaInfoHook:
    ...
    def _get_cursor(self):
        ...
        try:
            ...
            cursor.execute("SET SESSION wait_timeout = 43200")
            yield cursor
        finally:
            if cursor:
                cursor.close()
    ...
    def get_source_infos(self, inf, file_name=None, table_name=None, seq_num=None, dest_path=None) -> List[SourceInfo]:
        try:
            ...
            for row in records:
                ...
                logger.debug("fileInfo : %s", source_info)
    ...
    def get_collect_provide_infos(self, interface_id):
        try:
            # commons_test 코드와 달리 where 절에 test 관련 인터페이스 필터하는 조건 필요
            sql = "SELECT * FROM vw_collect_provide_infos WHERE collect_interface_id = %s and provide_use_yn ='Y' and provide_interface_id not like '%%-TEST-%%'"
