...
class DbConnWriter:
    ...
    def write_to_local_file(self, kafka_send, collect_history_dict, local_full_path, file_name, sql_result, total_batch):
        ...
        try:
            ...
            if total_batch % 250000 == 0:
                logger.debug("writing sql_result to local, collect_history_dict.id: %s, total_batch: %s, src: %s, path: %s", collect_history_dict.collect_hist_id, total_batch, file_name, local_full_path)
        except Exception as e:
            logger.error("Error writing sql_result to local, collect_history_dict.id: %s, src: %s,  path: %s, error: %s", collect_history_dict.collect_hist_id, file_name, local_full_path, e)
    ...
    def write_to_info_file(self, kafka_send, collect_history_dict, local_info_file_path, local_full_path, hdfs_path, file_name, ulid):
        ...
        try:
            ...
            logger.error("Error writing info to InfoDat, collect_history_dict.id: %s, src: %s, path: %s, error: %s", collect_history_dict.collect_hist_id, file_name, local_info_file_path, e)
