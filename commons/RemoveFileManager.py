...
from commons.HdfsUtil import remove_files, is_file_path, remove_diretory
from commons.MetaInfoHook import SparkServerInfo
...
class RemoveFileManager:
    ...
    def _delete_local(self):
        try:
            self.remove_file_path = os.path.dirname(self.remove_file_path)
            if os.path.exists(self.remove_file_path):
                shutil.rmtree(self.remove_file_path)
