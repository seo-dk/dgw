...
class PcellJarExecutor:
    def __init__(self, interface_info: InterfaceInfo, info_dat_path, partitions, kafka_send, collect_history):
        ...
        self.java_env = os.environ.copy()
        self.java_env["PATH"] += os.pathsep +"/usr/bin:/local/JAVA/bin"
    ...
    def start(self, file_info):
        ...
        proc = subprocess.Popen(self.command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=work_dir, env=self.java_env)
