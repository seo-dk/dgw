...
def get_mobile_numbers():
    usernames = Variable.get("sms_target_usernames", default_var=[], deserialize_json=True)
    if not usernames:
        return []
...
def send_sms_alert(context):
    ...
    ignore_dag_ids = Variable.get("sms_ignore_dags", default_var=[], deserialize_json=True)    
    if ti.dag_id in ignore_dag_ids:
        logger.warn(f"[SMS SKIP] {ti.dag_id} is in ignore list")
        return

    local_tz = pendulum.timezone("Asia/Seoul")
    exec_date = context.get('execution_date')
    exception = context.get('exception')
    when = ti.end_date.astimezone(local_tz).strftime("%-m/%-d %-H:%-M:%-S")

    msg=f"[{when}] ({ti.try_number -1}) {ti.dag_id}, {exception}, {ti.hostname}"
...
def simple_send(to_number: str, msg: str):
    send(
        from_number = '0317105551', 
        to_number = to_number, 
        msg = msg, 
        nms = 'none', 
        tsp = 'skt', 
        server_info = DEFAULT_SERVERS['skt'], 
        socket_timeout = 5.0
    )
...
def send(
        ...
        server_info: Dict[str, Tuple[str, int]]
    ) -> bool:
