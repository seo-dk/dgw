...
class DagGenerator:
    ...
    def _get_replacements(self, row):
        ...
        # if 'TPANI-DISTCP-OD' in interface_id:
        #     recipient_emails.append('kdseo@mobigen.com')

        # if 'OCGW' in interface_id:
        #     recipient_emails = []
        ...
        return {
            ...
            "$MAX_ACTIVE_RUNS": int(row.get('MAX_ACTIVE_RUNS', 6)),
            "$MAX_ACTIVE_TASKS": int(row.get('MAX_ACTIVE_TASKS', 6)),
            "$RETRIES": int(row.get('RETRIES', 2)),
            "$IGNORE_SMS_ALERT": bool(row.get('IGNORE_SMS_ALERT', False)),
        }
