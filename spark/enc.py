...
def main():
    try:
        ...
        ciphers_keys = decode_to_json(sys.argv[3])
        enc_rules = decode_to_json(sys.argv[4])
        file_format = sys.argv[5]
        compression = sys.argv[6]
        tb_name = sys.argv[7]
        target_format = sys.argv[8]
        delimiter = bytes(sys.argv[9], "utf-8").decode("unicode_escape")
        encoding = sys.argv[10]
        schema = decode_to_json(sys.argv[11])
        crypto_type = sys.argv[12]
        merge = sys.argv[13]
