...
# spark properties
# spark-submit 이후 20초뒤 삭제
# VAULT_TOKEN은 일회용으로 재사용 불가
PROPS=$(mktemp /dev/shm/.sp-XXXXXX)  # unique id
chmod 600 $PROPS
cat > $PROPS <<EOF
spark.yarn.appMasterEnv.VAULT_TOKEN=$VAULT_TOKEN
spark.yarn.appMasterEnv.VAULT_ADDR=$VAULT_ADDR
spark.yarn.appMasterEnv.VAULT_IP=$VAULT_IP
spark.yarn.appMasterEnv.VAULT_NAMESPACE=$VAULT_NAMESPACE
spark.yarn.appMasterEnv.VAULT_SECRET_PATH=$VAULT_SECRET_PATH
spark.yarn.appMasterEnv.ENC_RULES=$ENC_RULES
EOF

# background에서 20초 대기 후 삭제
(sleep 20 && rm -f $PROPS) &
...
spark-submit \
  ...
  data_processing.py "$input_path" "$output_path" "$file_format" "$target_compression" "$tb_name" "$target_format" "$delimiter" "$encoding" "$ENCODED_SCHEMA" "$crypto_type" "$merge"

echo "submit finished"
echo "$path_identifier"
yarn application -list -appStates ALL | grep "$path_identifier" | sort -k1 | tail -n1 >> enc.log
...
if [ -z "$app_id" ]; then
    echo "$current_time - Failed to retrieve application ID." >> enc.log
    exit 1
fi

if [ "$app_status" == "SUCCEEDED" ]; then
    echo "$current_time - YARN application $app_id completed successfully." >> enc.log
    exit 0
elif [ "$app_status" == "FAILED" ]; then
    echo "$current_time - YARN application $app_id failed." >> enc.log
    exit 1
else
    echo "$app_status - YARN application $app_id killed." >> enc.log
    exit 2
fi
