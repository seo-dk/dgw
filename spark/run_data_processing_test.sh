...
spark-submit \
  ...
  data_processing_test.py "$input_path" "$output_path" "$file_format" "$target_compression" "$tb_name" "$target_format" "$delimiter" "$encoding" "$ENCODED_SCHEMA" "$crypto_type" "$merge"
