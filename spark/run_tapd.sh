...
spark-submit \
  ...
  enc.py "$input_path" "$output_path" "$ciphers_keys" "$enc_rules" "$file_format" "$target_compression" "$tb_name" "$target_format" "$delimiter" "$encoding" "$ENCODED_SCHEMA" "$crypto_type" "$merge"
